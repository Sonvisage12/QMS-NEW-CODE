        ...
        if (strcmp(item.uid, "REQ_NEXT") == 0) {
            ...
            if (!sharedQueueC.empty()) {
               ...
               if (sharedQueue.exists(entry.uid)) {
                    sharedQueue.pop();
                    //sharedQueue.insertAt(2, entry.uid, entry.timestamp, entry.number); // insert into main queue
                     sharedQueue.insertAt(10, entry.uid, entry.timestamp, entry.number);  
                    sharedQueueC.insertAt(10, entry.uid, entry.timestamp, entry.number); 
                    
                     Serial.println("📍 After InsertAt:");
        std::vector<QueueEntry> entries = sharedQueue.getAll();
        for (size_t i = 0; i < entries.size(); ++i) {
            Serial.printf("Position %d: UID %s\n", (int)i, entries[i].uid.c_str());
                }
                }
                 else if (sharedQueueB.exists(entry.uid)) {
                    sharedQueueB.pop();
                    //sharedQueueB.insertAt(2, entry.uid, entry.timestamp, entry.number); // insert into queue B
                    sharedQueueB.insertAt(4, entry.uid, entry.timestamp, entry.number);
                    sharedQueueC.insertAt(6, entry.uid, entry.timestamp, entry.number); 
                }
...
void createMixedQueue() {
    ...
    while (indexA < entriesA.size() || indexB < entriesB.size()) {
        ...
        if (indexA < entriesA.size()) {
            for (int i = 0; i < 2 && indexA < entriesA.size(); i++, indexA++) {
